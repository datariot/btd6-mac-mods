...
internal static class Il2CppHandler
{
    ...
    {
        // Port-hardening: reject an invoke whose target methodPointer (MethodInfo
        // field @ offset 0) is non-executable, so a Windows-only mod method that's
        // absent from the macOS game build degrades to null instead of a SIGBUS.
        if (MethodPointerGuard.Enabled && method != 0)
        {
            var methodPointer = Marshal.ReadIntPtr(method);
            if (!MethodPointerGuard.IsExecutable(methodPointer))
            {
                MethodPointerGuard.ReportBlocked(method, methodPointer, il2cpp.GetMethodName(method));
                if (exc != 0)
                    Marshal.WriteIntPtr(exc, 0);
                return 0;
            }
        }
