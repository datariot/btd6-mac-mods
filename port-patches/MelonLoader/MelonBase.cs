...
#pragma warning restore CS0612 // Type or member is obsolete
        ...
        internal void UnregisterInstance(string reason, bool silent)
        {
            ...
            // macOS root-cause fix (same rationale as Core.Quit): restoring a mod's patched __TEXT bytes
            // via HarmonyInstance.UnpatchSelf() is unsafe on the Apple-Silicon port and SIGBUSes inside
            // MLMacOSJitCopy. On the Unity/in-game quit path this per-mod unpatch runs (via
            // OnApplicationDefiniteQuit -> UnregisterMelons) BEFORE Core.Quit's guard, so it must be
            // skipped here too. Mods on this port never hot-unload, so the only caller in practice is
            // shutdown, where a dying process has nothing to restore. Skip the unpatch on macOS.
