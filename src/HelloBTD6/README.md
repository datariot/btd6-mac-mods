...
- **Reference HintPaths wrong** — the `UnityEngine.*` / `MelonLoader.dll` paths differ between
  MelonLoader versions (sometimes `net6/`, sometimes `Managed/`, IL2CPP interop under
  `Il2CppAssemblies/`). The build error tells you the missing file; fix the HintPath.
